...
async def summ(ctx, name, region):
    try:
        ...
        if region == 'EUW' or region =='NA' or region == 'TR':
            ...
            for i in range(3):
                embedFile.add_field(name = "Champion-", value = get_champions(masteries[i]['championId']) , inline = True)
                embedFile.add_field(name = "Level", value = masteries[i]['championLevel'] , inline = True)
                embedFile.add_field(name = "Points", value = masteries[i]['championPoints'] , inline = True)
